...
export default function ModalDiscapacidades({ estudianteId, discapacidadesActuales, onClose }: Props) {
  ...
  const agregar = () => {
    ...
    const disponible = catalogo.find((c: { id: number }) => !yaUsados.has(c.id))
    if (!disponible) { toast.error('Ya están todos los tipos agregados'); return }
  ...
  const cambiarTipo = (i: number, id: number) =>
    setLista(p => p.map((d, idx) => idx === i ? { ...d, tipoDiscapacidadId: id } : d))
  const cambiarObs = (i: number, val: string) =>
    setLista(p => p.map((d, idx) => idx === i ? { ...d, observaciones: val || undefined } : d))
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <h2 style={{ color: '#F4E9CD' }} className="text-sm font-semibold">Discapacidades</h2>
        ...
        <div className="p-5 space-y-3 max-h-96 overflow-y-auto">
          {lista.length === 0 && (
            <p style={{ color: '#475569' }} className="text-sm text-center py-4">Sin discapacidades registradas</p>
          )}
          {lista.map((d, i) => (
            <div key={i} className="space-y-2">
              <div className="flex gap-2 items-center">
                ...
                <button onClick={() => quitar(i)} style={{ color: '#f87171' }} className="px-2 py-2 hover:opacity-70 transition text-sm">✕</button>
              </div>
              <input
                type="text"
        ...
        <div style={{ borderTop: '1px solid #1e293b' }} className="px-5 py-4 flex justify-end gap-3">
          <button onClick={onClose} style={{ background: '#0f172a', color: '#94a3b8', border: '1px solid #334155' }} className="px-4 py-2 rounded-lg text-sm font-medium hover:opacity-80 transition">Cancelar</button>
          <button onClick={() => mut.mutate()} disabled={mut.isPending} style={{ background: mut.isPending ? '#c9c0a8' : '#F4E9CD', color: '#0f172a' }} className="px-4 py-2 rounded-lg text-sm font-medium hover:opacity-90 transition">
