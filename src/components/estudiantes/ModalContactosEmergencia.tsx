...
export default function ModalContactosEmergencia({ estudianteId, contactosActuales, onClose }: Props) {
  ...
  const cambiar = (i: number, campo: keyof ContactoEmergenciaRequest, val: string) =>
    setLista(p => p.map((c, idx) =>
      idx === i ? { ...c, [campo]: val || undefined } : c
    ))

  const guardar = () => {
    const invalidos = lista.filter(c =>
      !c.nombreCompleto.trim() || !c.parentesco.trim()
    )
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <h2 style={{ color: '#F4E9CD' }} className="text-sm font-semibold">Contactos de emergencia</h2>
        ...
        <div className="p-5 space-y-4 max-h-112 overflow-y-auto">
          {lista.map((c, i) => (
            ...
            >
              ...
              <p style={{ color: '#475569' }} className="text-xs font-medium uppercase tracking-wider mb-3">
                Contacto {i + 1}
              </p>

              <div className="grid grid-cols-2 gap-2">
                <div className="col-span-2">
                  <label style={{ color: '#94a3b8' }} className="block text-xs mb-1">Nombre completo *</label>
                  <input
                    type="text"
                ...
                <div>
                  <label style={{ color: '#94a3b8' }} className="block text-xs mb-1">Parentesco *</label>
                  <input
                    type="text"
                ...
                <div>
                  <label style={{ color: '#94a3b8' }} className="block text-xs mb-1">Dirección</label>
                  <input
                    type="text"
