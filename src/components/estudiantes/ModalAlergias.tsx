...
export default function ModalAlergias({ estudianteId, alergiasActuales, onClose }: Props) {
  ...
  const agregar = () => {
    ...
    if (!disponible) { toast.error('Ya están todas las alergias agregadas'); return }
  ...
  const cambiarObs = (i: number, val: string) =>
    setLista(p => p.map((a, idx) => idx === i ? { ...a, observaciones: val || undefined } : a))
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <h2 style={{ color: '#F4E9CD' }} className="text-sm font-semibold">Alergias</h2>
        ...
        <div className="p-5 space-y-3 max-h-96 overflow-y-auto">
          {lista.length === 0 && (
            <p style={{ color: '#475569' }} className="text-sm text-center py-4">
