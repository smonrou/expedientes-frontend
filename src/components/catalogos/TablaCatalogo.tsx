 ...
 */
export default function TablaCatalogo({ items, onEditar, onEliminar, soloLectura }: Props) {
  if (items.length === 0) {
    return (
      <div className="flex items-center justify-center py-16">
        <p className="text-sm" style={{ color: '#475569' }}>No hay registros aún.</p>
      </div>
    )
  }
        ...
        <thead>
          <tr style={{ borderBottom: '1px solid #1e293b' }}>
            <th className="text-left px-4 py-3 text-xs font-medium uppercase tracking-wider"
              style={{ color: '#475569' }}>ID</th>
            <th className="text-left px-4 py-3 text-xs font-medium uppercase tracking-wider"
              style={{ color: '#475569' }}>Nombre</th>
            {!soloLectura && (
              <th className="text-right px-4 py-3 text-xs font-medium uppercase tracking-wider"
                style={{ color: '#475569' }}>Acciones</th>
            )}
          </tr>
        </thead>
        <tbody>
          {items.map((item, i) => (
            ...
            >
              ...
              <td className="px-4 py-3" style={{ color: '#cbd5e1' }}>{item.nombre}</td>
              {!soloLectura && (
