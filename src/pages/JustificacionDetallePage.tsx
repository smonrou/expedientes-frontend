...
import {
  obtenerJustificacion, cambiarEstado,
...
export default function JustificacionDetallePage() {
  ...
  const eliminarDocMut = useMutation({
    mutationFn: (documentoId: number) => eliminarDocumento(Number(id), documentoId),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['justificacion', id] })
  ...
  const puedeSubirDoc = !['APROBADA', 'RECHAZADA'].includes(just.estado) &&
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        {tieneRol('ADMIN', 'COORDINADOR') && transicionesPosibles.length > 0 && (
          <div className="flex gap-2">
            {transicionesPosibles.map(nuevoEstado => (
              <button
                ...
                style={{
                  background: nuevoEstado === 'RECHAZADA'
                    ? 'rgba(248,113,113,0.15)'
                    : nuevoEstado === 'APROBADA'
                    ? 'rgba(34,197,94,0.15)'
                    : 'rgba(244,233,205,0.1)',
                  color: nuevoEstado === 'RECHAZADA'
                    ? '#f87171'
                    : nuevoEstado === 'APROBADA'
                    ? '#22c55e'
                    : '#F4E9CD',
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        <Seccion titulo="Detalle">
          ...
          <Campo
            label="Presentada"
            valor={format(new Date(just.fechaPresentacion), "dd 'de' MMMM yyyy, HH:mm", { locale: es })}
          />
          {just.fechaRevision && (
            <Campo
              label="Revisada"
              valor={format(new Date(just.fechaRevision), "dd 'de' MMMM yyyy, HH:mm", { locale: es })}
            />
          )}
          ...
          <div>
            <p className="text-xs mb-1" style={{ color: '#475569' }}>Descripción</p>
      ...
      <Seccion titulo="Documentos adjuntos">
        <div className="space-y-2">
          {just.documentos.length === 0 && (
            <p className="text-xs" style={{ color: '#475569' }}>Sin documentos adjuntos.</p>
          )}
          {just.documentos.map(doc => (
            ...
            >
              <div>
                ...
                <p className="text-xs mt-0.5" style={{ color: '#475569' }}>
                  {format(new Date(doc.subidoEn), 'dd MMM yyyy, HH:mm', { locale: es })}
                </p>
              </div>
              <div className="flex items-center gap-2">
                <button
...
function Campo({ label, valor }: { label: string; valor: string }) {
  return (
    <div className="flex items-start justify-between gap-4">
      ...
      <p className="text-xs text-right" style={{ color: '#cbd5e1' }}>{valor}</p>
    </div>
  )
}