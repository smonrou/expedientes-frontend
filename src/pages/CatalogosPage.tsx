...
export default function CatalogosPage() {
  ...
  const [itemEditando, setItemEditando] = useState<
    CatalogoResponse | CarreraResponse | null
  >(null);
  ...
  const crearMut = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["catalogo", tabActiva] });
      cerrarModal();
  ...
  const crearCarreraMut = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["catalogo", "carreras"] });
      cerrarModal();
  ...
  const actualizarCarreraMut = useMutation({
    mutationFn: ({ id, dto }: { id: number; dto: CarreraRequest }) =>
      actualizarCarrera(id, dto),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["catalogo", "carreras"] });
      cerrarModal();
  ...
  const mutCargando =
    ...
    crearCarreraMut.isPending ||
    actualizarCarreraMut.isPending;
