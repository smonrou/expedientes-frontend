...
import {
  ...
  cambiarContrasena, activarUsuario, desactivarUsuario,
} from '@/api/usuarios'
...
export default function UsuariosPage() {
  ...
  const crearMut = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['usuarios'] }); cerrarModales(); toast.success('Usuario creado') },
    onError: () => toast.error('Error al crear el usuario'),
  })

  const actualizarMut = useMutation({
    mutationFn: ({ id, dto }: { id: number; dto: Parameters<typeof actualizarUsuario>[1] }) =>
      actualizarUsuario(id, dto),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['usuarios'] }); cerrarModales(); toast.success('Usuario actualizado') },
  ...
  const contrasenaMut = useMutation({
    ...
    onSuccess: () => { cerrarModales(); toast.success('Contraseña actualizada') },
    onError: () => toast.error('Error al cambiar la contraseña'),
  })

  const activarMut = useMutation({
    mutationFn: activarUsuario,
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['usuarios'] }); toast.success('Usuario activado') },
  ...
  async function handleGuardarContrasena(nuevaContrasena: string) {
    ...
    await contrasenaMut.mutateAsync({ id: usuarioSeleccionado.id, nuevaContrasena })
  }

  function toggleActivar(u: UsuarioResponse) {
    const accion = u.activo ? 'desactivar' : 'activar'
    if (!confirm(`¿${accion.charAt(0).toUpperCase() + accion.slice(1)} a ${u.nombreUsuario}?`)) return
    if (u.activo) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-semibold" style={{ color: '#F4E9CD' }}>Usuarios</h1>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr style={{ borderBottom: '1px solid #1e293b' }}>
                  {['Usuario', 'Correo', 'Rol', 'Estado', 'Creado', 'Acciones'].map(col => (
                    <th
                      key={col}
              ...
              <tbody>
                {usuarios.map((u, i) => (
                  <tr
                    key={u.id}
                    style={{ borderBottom: i < usuarios.length - 1 ? '1px solid #1e293b' : 'none' }}
                  ...
                  >
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span style={{ color: '#cbd5e1' }}>{u.nombreUsuario}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-xs" style={{ color: '#94a3b8' }}>{u.correo}</td>
                    <td className="px-4 py-3">
                      <span
                    ...
                    <td className="px-4 py-3">
                      <span
                        className="px-2 py-0.5 rounded-md text-xs font-medium"
                        style={{
                          background: u.activo ? 'rgba(34,197,94,0.1)' : 'rgba(248,113,113,0.1)',
