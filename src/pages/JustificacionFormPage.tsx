...
import { listarEstudiantes, obtenerMiExpediente } from "@/api/estudiantes";
import { useAuth } from "@/hooks/useAuth";
import type { Resolver } from "react-hook-form";

const schema = z.object({
  estudianteId: z.coerce.number().optional(),
 ...
 * Para ESTUDIANTE obtiene su propio estudianteId desde mi-expediente.
 * Para ADMIN permite seleccionar cualquier estudiante.
 */
export default function JustificacionFormPage() {
  ...
  // Para ESTUDIANTE: obtener su estudianteId real desde el expediente
  const { data: miExpediente } = useQuery({
    queryKey: ["mi-expediente", sesion?.usuarioId],
    queryFn: () => obtenerMiExpediente(sesion!.usuarioId),
    enabled: tieneRol("ESTUDIANTE") && !!sesion?.usuarioId,
  });
  ...
  const crearMut = useMutation({
    mutationFn: (data: FormData) => {
      // Para ESTUDIANTE usar el id del expediente, no el usuarioId
      const estudianteId = tieneRol("ESTUDIANTE")
        ? miExpediente!.id
        : data.estudianteId!;
      return crearJustificacion(estudianteId, {
    ...
    onError: (error: { response?: { status: number } }) => {
      if (error.response?.status === 409) {
        toast.error(
          "Una o más fechas ya están cubiertas por otra justificación",
        );
      } else {
  ...
  function onSubmit(data: FormData) {
    ...
    if (tieneRol("ESTUDIANTE") && !miExpediente) {
      toast.error("Cargando datos del expediente, intenta de nuevo");
      return;
    }
    if (tieneRol("ADMIN") && (!data.estudianteId || data.estudianteId < 1)) {
...
function FormSeccion({
  ...
  return (
    <div
      className="rounded-xl p-5 space-y-4 mb-4"
