 ...
 */
export async function listarNoLeidas(
  destinatarioId: number,
): Promise<NotificacionResponse[]> {
  const { data } = await api.get<NotificacionResponse[]>(
 ...
 */
export async function conteoNoLeidas(
  destinatarioId: number,
): Promise<ConteoNoLeidasResponse> {
...
export async function marcarTodasLeidas(destinatarioId: number): Promise<void> {
  await api.patch(
    `/notificaciones/usuario/${destinatarioId}/marcar-todas-leidas`,
  );
}
